...
class GmailEmailSender:
    ...
    async def send_registration_email_with_pdf(
        ...
        try:
            # Generate PDF
            pdf_bytes = pdf_generator.generate_qr_tickets_pdf(qr_codes_data, settings.event_name)
            
            # Create email content
            subject = f"Welcome to {settings.event_name} - Your QR Code Tickets Inside!"
            html_content = self.create_registration_email_with_pdf_content(name, len(qr_codes_data), total_price)
    ...
    def create_registration_email_with_pdf_content(self, name: str, ticket_count: int, total_price: float) -> str:
